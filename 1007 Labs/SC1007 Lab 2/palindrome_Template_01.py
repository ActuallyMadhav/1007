...
def palindrome(word):
    ...
    # Push characters onto the stack (ignoring spaces and converting to uppercase)
    for char in word:
        if char != ' ':
    ...
    for char in word:
        if char != ' ':
            # Pop the character from the stack and compare with the current character
            if char.upper() != stack.pop():
                print("The string is not a palindrome")
                return
